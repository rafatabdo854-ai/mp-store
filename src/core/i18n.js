...
const PATTERNS = [
  ...
  [/^(\S+) صنف$/, (m) => `${m[1]} items`],
  [/^(\S+) مستخدم$/, (m) => `${m[1]} users`],
  [/^(\S+) متصل الآن$/, (m) => `${m[1]} online now`],
  [/^(\S+) نوافذ$/, (m) => `${m[1]} windows`],
  ...
  // نصوص لوحة القيادة والقوائم — تُبنى وقت التشغيل فلا يمكن وضعها
  // في القاموس كما هي
  [/^(\S+) صنف نفد بالكامل$/, (m) => `${m[1]} items completely out of stock`],
  [/^(\S+) صنف تحت الحد الأدنى$/, (m) => `${m[1]} items below minimum`],
  [/^(\S+) صنف له رصيد بلا سعر$/, (m) => `${m[1]} items in stock without a price`],
  [/^(\S+) صنف مسعّر$/, (m) => `${m[1]} items priced`],
  [/^(\S+) وحدة في المخزن$/, (m) => `${m[1]} units in store`],
  [/^(\S+) وحدة$/, (m) => `${m[1]} units`],
  [/^(\S+)% من الأصناف$/, (m) => `${m[1]}% of items`],
  [/^(\S+) من (\S+) صنف$/, (m) => `${m[1]} of ${m[2]} items`],
  [/^عرض (\S+) من (\S+) صنف$/, (m) => `Showing ${m[1]} of ${m[2]} items`],
  [/^عرض (\S+)–(\S+) من (\S+) حركة$/,
    (m) => `Showing ${m[1]}–${m[2]} of ${m[3]} transactions`],
  [/^عرض (\S+)–(\S+) من (\S+) صنف$/,
    (m) => `Showing ${m[1]}–${m[2]} of ${m[3]} items`],
  [/^(\S+) حركة$/, (m) => `${m[1]} transactions`],
  [/^(\S+) إذن$/, (m) => `${m[1]} vouchers`],
  [/^(\S+) يوم$/, (m) => `${m[1]} days`],
  [/^آخر تحديث (.+)$/, (m) => `Last updated ${m[1]}`],
  [/^من (.+?) إلى (.+?) — (\S+) صنف متحرّك أو له رصيد$/,
    (m) => `From ${m[1]} to ${m[2]} — ${m[3]} items with movement or stock`],
  [/^(.+?) ج\.م$/, (m) => `${m[1]} EGP`],
  [/^(\S+) مشروع$/, (m) => `${m[1]} projects`],
  [/^(\S+) مورد$/, (m) => `${m[1]} suppliers`],
  [/^(\S+) فئة$/, (m) => `${m[1]} categories`],
  [/^(\S+) سجل$/, (m) => `${m[1]} records`],
];
...
export function t(text) {
  ...
  // احتياطي: نصّ يبدأ أو ينتهي برقم وبقيّته عبارة معروفة.
  // يلتقط ما لم ننتبه له من النصوص المركّبة دون قاعدة لكل واحدة.
  const head = key.match(/^([\d٠-٩][\d٠-٩,.\s%–-]*?)\s+(\D.*)$/);
  if (head && EN[head[2]]) return westernize(head[1]) + " " + EN[head[2]];

  const tail = key.match(/^(.*?\D)\s+([\d٠-٩][\d٠-٩,.\s%–-]*)$/);
  if (tail && EN[tail[1]]) return EN[tail[1]] + " " + westernize(tail[2]);
